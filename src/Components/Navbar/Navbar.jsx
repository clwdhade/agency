  ...
  return (
    <div className={`flex justify-between items-center px-12 py-4 bg-slate-300 fixed top-0 w-full z-50 ${responsive ? 'flex-col items-center' : ''}`}>
      ...
      <ul className={`nav_menu md:flex ${responsive ? 'flex flex-col gap-4 h-[50dvh] transition-all ease-in-out justify-center items-center' : 'flex-row gap-4'} text-xl ${responsive ? '' : 'hidden'}`}>
        <li className="hover:underline-offset-2 hover:underline cursor-pointer decoration-yellow-500 transition-all ease-in-out" onClick={closeNavbar}>
          <Link to="/">Home</Link>
        </li>
        <li className="hover:underline-offset-2 hover:underline cursor-pointer decoration-yellow-500 transition-all ease-in-out" onClick={closeNavbar}>
        ...
        <BsXCircle
          className={`cursor-pointer text-2xl hover:red-400 md:hidden transition-all ease-in-out ${responsive ? 'block' : 'hidden'}`}
          onClick={toggleResponsive}
        />
      </ul>
      <FaBars
        className={`cursor-pointer text-xl md:hidden transition-all ease-in-out ${responsive ? 'hidden' : 'block'}`}
        onClick={toggleResponsive}
      />
    </div>
  );
